...
- **Value parameters accept expressions.** Anything marked *expr* below can be a literal, a `{{ reference }}`, or a computed `{{= JSONLogic =}}` expression — see [References & expressions](/workflows/expressions).
...
To pass a structured input, build a record with the `recordOf` operator:

```json
{ "recordOf": ["invoiceId", { "var": "document.kw.InvoiceNumber.0" }, "amount", 1000] }
```
