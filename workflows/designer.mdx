...
- **Guard** — an optional [JSONLogic condition](/workflows/expressions); the transition is only available when it evaluates truthy. Guards are evaluated once, when the instance enters the node.
...
3. The trigger's optional **filter** (raw JSONLogic against the event) evaluates truthy — no filter means every in-scope event of that type starts an instance
