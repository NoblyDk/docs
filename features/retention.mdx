...
Retention is the part of Nobly Insight that deletes documents on a schedule, so a customer can meet GDPR and other regulated retention rules without anyone deleting documents by hand. You describe *which* documents to delete and *how long* to keep them as **policies**; Nobly Insight evaluates those policies on a recurring schedule and deletes what's eligible.

This page is a consultant and administrator reference: it explains how retention works end to end, how to configure policies, how processing is scheduled, and how to verify a setup before it goes live. Every retention screen also carries inline help in the same shape as the rest of the platform.

<Warning>
  Retention performs **recoverable** deletion under a **Soft delete** policy and **irreversible** deletion under a **Permanent purge** policy. Turn it on deliberately: build the first policy disabled, simulate it, review the result with the customer, and only then enable it. Never go straight from creating a policy to enabling it on a live tenant.
</Warning>

## How it works

Each document a policy is tracking moves through a series of states. You'll recognise these states when supporting a customer or reading a document's retention status.
...
A policy is the unit of "delete these documents after this long". You'll typically build the first few policies during a deployment, then hand the editor to the customer's super-users.
...
|---|---|
| **Name / Description** | Identification for the policy. Agree a naming convention with the customer up front |
...
| **Deletion method** | **Soft delete** (recoverable, default) or **Permanent purge** (irreversible). Only use Permanent purge when the customer's regulator requires content to be destroyed |
...
| **Approver groups** | The user groups whose members can approve or reject deletions for this policy. Map to existing customer groups |
...
When the sample is capped, read the **total** to judge real impact, not the size of the sample. Walk through both with the customer before enabling the policy.
...
- Approvers can sign off the whole batch, or **exclude individual documents** from it without affecting the rest — useful when the customer wants to keep one specific document.
...
There are three ways to stop a document from being deleted. Each maps to a different need, so make sure customer admins know which to use.
...
- **It can be turned off independently.** While processing is off, you and the customer can still author policies, run simulations, place legal holds, and review everything in the UI — but no documents are tagged or deleted. This is the safe default until the first policy is signed off.
...
- **The schedule is set per tenant.** Place it in the customer's quiet hours, clear of business hours, backups, and any indexing or AI processing.

Processing runs automatically on its schedule; there's no manual "run now" button in the product UI. If a customer needs deletion to happen sooner, adjust the schedule.

## Access and permissions

Retention access is granted per capability to the customer's **user groups**, under **Settings → Access → Permissions**. A user receives the combined permissions of every group they belong to. Changes to a group's grants take effect within a few minutes.
...
The Retention section opens on a **Dashboard** that gives the customer an at-a-glance view of retention activity:
...
<Steps>
  <Step title="Confirm retention is available">
    Retention is provisioned per tenant. Confirm it's switched on for the customer before you begin.
  </Step>
  <Step title="Grant access">
    Map the retention capabilities onto the customer's user groups under **Settings → Access → Permissions**. At minimum, the people authoring policies need policy-manage and simulate.
  </Step>
  ...
  <Step title="Simulate">
    Run a simulation. Review the matched total and the sample with the customer, and confirm the policy targets exactly what they expect.
  </Step>
  <Step title="Confirm the schedule">
    Make sure the processing window sits in the customer's quiet hours, clear of business hours, backups, and indexing.
  </Step>
